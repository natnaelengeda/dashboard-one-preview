...
export const SimpleMantine = ({ elements, cat, rom }) => {
  ...
  const rows = elements.map((element) => (
    <tr key={element.id}>
      ...
      <td className="w-32 text-sm">{element.sub_code}</td>
      <td>{element.location_code}</td>
      <td>{element.owner}</td>
      <td>{element.entrust_id}</td>
      <td>{element.price}</td>
        ...
        <div className="flex flex-col items-center justify-center ">
          ...
          {catagory && (
            <div className="flex flex-row items-end gap-2">
              <h1 className="text-xl">Asset Type:- </h1>{" "}
              <h1 className="text-lg font-bold">{catagory}</h1>
            </div>
          )}
          {room && (
            <div className="flex flex-row items-end gap-2">
